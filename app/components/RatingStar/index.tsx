...
export default function RatingStar(props: IRatingStarProps): JSX.Element {
    ...
    const arrStars = [{ from: 1 }, { from: 2 }, { from: 3 }, { from: 4 }, { from: 5 }];
        ...
        <View style={{ flexDirection: 'row' }}>
            {arrStars.map((item, index) => (
                <MaterialCommunityIcons
                    ...
                    name="star"
                    size={props.starSize}
                    color={props.rating >= item.from ? '#FFC107' : '#BDBDBD'}
                />
            ))}
        </View>
    );
